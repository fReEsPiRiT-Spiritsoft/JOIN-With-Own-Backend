 ...
 * Handles task form population, validation, and updates via the API.
 */
@Component({
...
export class TaskCardEdit implements OnInit, OnChanges {
  ...
  /**
   * Populates the due date fields from the task's date string.
   * Converts to DD/MM/YYYY format for display and YYYY-MM-DD for input.
   */
  private populateDueDate() {
    if (!this.task?.dueDate) return;
    const date = this.parseDate(this.task.dueDate);
  ...
  /**
   * Parses a date string (ISO or DD/MM/YYYY) into a Date.
   */
   ...
   * Updates the task via the API and emits the updated task.
   */
  async onSubmit() {
